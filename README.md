...
**Alpha.** Manual editing is the starting point. Desktop AI dispatch and clipboard paste have reported bugs; use **Open** to load an image. Prebuilt downloads are not available yet.
...
On Linux, install `scripts/flameshot-moneyshot` somewhere on your `PATH` and bind it to your screenshot shortcut. It opens Flameshot for region selection, keeps the capture in the system clipboard, and opens the same image in Moneyshot for annotation.
